...
When a limit has a '-' sign it approaches from the \textit{left}.\\
When a limit has a '+' sign it approaches from the \textit{right}.\\
\\

\section{Trigonometric Functions}
There are four important limit functions:
\begin{enumerate}
\item $\lim\limits_{x \to c} sinx = sinc$\\
\item $\lim\limits_{x \to c} cosx = cosc$\\
\item $\lim\limits_{x \to 0} \frac{sinx}{x} = 1$\\
\item $\lim\limits_{x \to 0} \frac{1-cosx}{x} = 0$
\end{enumerate}
\section{Continunity}
...
\begin{enumerate}
\item A limit exists at $f(b)$ which is 4.\\
\item The limit \textit{at} $f(b)$ is 1 aka 'a'.\\
\item This line has a disconnected continunity. \\
\end{enumerate}
